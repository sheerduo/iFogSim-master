...
public class CloudletSchedulerTimeShared extends CloudletScheduler {
	...
	public List<Double> getCurrentRequestedMips() {
		...
		//System.out.println("used   "+mipsShare);
		return mipsShare;
	}
	...
	/**
	 * LIKAI
	 * return the sum cpuLength of all the cloudlet in executing
	 * @return
	 */
	@Override
	public Pair<Double, Double> getRestCloudletCpuLength(){
		double min= Double.MAX_VALUE;
		double result=0;
		for(ResCloudlet rc :getCloudletExecList()){
			double md = rc.getRemainingCloudletLength();
			if(md < min)
				min = md;
			result += md;
		}
		return new Pair<>(min, result);
	}
}
