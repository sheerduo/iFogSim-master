...
public class DCNSFogAndHealthCare {
        ...
        try {
            ...
            for(FogDevice device : fogDevices){
                if(device.getName().startsWith("md")){
                    ...
                    moduleMapping_d.addModuleToDevice("object_detector", device.getName());
                    moduleMapping_d.addModuleToDevice("object_tracker", device.getName());
                    moduleMapping_h.addModuleToDevice("Client", device.getName());
                    moduleMapping_h.addModuleToDevice("Data_filtering", device.getName());
                    moduleMapping_h.addModuleToDevice("Data_processing", device.getName());
                    moduleMapping_h.addModuleToDevice("Event_handler", device.getName());
                ...
                if(device.getName().startsWith("d")){
                    moduleMapping_h.addModuleToDevice("Client", device.getName());
                    moduleMapping_h.addModuleToDevice("Data_filtering", device.getName());  // fixing all instances of the Client module to the Smartphones
                    moduleMapping_h.addModuleToDevice("Data_processing", device.getName());
                    moduleMapping_h.addModuleToDevice("Event_handler", device.getName());
                    moduleMapping_d.addModuleToDevice("motion_detector", device.getName());
                    moduleMapping_d.addModuleToDevice("object_detector", device.getName());
                    moduleMapping_d.addModuleToDevice("object_tracker", device.getName());
                    //moduleMapping_1.addModuleToDevice("client_1", device.getName());  // fixing all instances of the Client module to the Smartphones
                }
            }
            moduleMapping_d.addModuleToDevice("user_interface", "cloud");
            /*moduleMapping_d.addModuleToDevice("motion_detector", "cloud");
            moduleMapping_d.addModuleToDevice("object_detector", "cloud");
            moduleMapping_d.addModuleToDevice("object_tracker", "cloud");*/
            /*moduleMapping_h.addModuleToDevice("Client", "cloud");
            moduleMapping_h.addModuleToDevice("Data_filtering", "cloud");
            moduleMapping_h.addModuleToDevice("Data_processing", "cloud");
            moduleMapping_h.addModuleToDevice("Event_handler", "cloud");*/
...
*/
    ...
    private static FogDevice addGw(String id, int parentId){
        FogDevice dept = createFogDevice("d-"+id, 3000, 4000, 10000, 10000, 2, 0.0, 107.339, 83.4333);
        fogDevices.add(dept);
    ...
    private static FogDevice addMobile_D(String id, int parentId){
        FogDevice mobile = createFogDevice("md-"+id, 2000, 1000, 10000, 270, 3, 0, 87.53, 82.44);
    ...
    private static FogDevice addMobile_H(String id, int parentId){
        FogDevice mobile = createFogDevice("mh-"+id, 2000, 2048, 10000, 270, 3, 0, 87.53, 82.44);
        mobile.setParentId(parentId);
    ...
    private static void createHealthCareDevices(int userId, String appId) {
        for(FogDevice mobile : mobiles_H){
            ...
            actuators.add(display);
            eegSensor.setGatewayDeviceId(mobile.getId());
            eegSensor.setLatency(2.0);  // latency of connection between EEG sensors and the parent Smartphone is 6 ms
            display.setGatewayDeviceId(mobile.getId());
            display.setLatency(2.0);  // latency of connection between Display actuator and the parent Smartphone is 1 ms
        }
    }
    ...
    private static Application createApplication2(String appId, int userId){
        ...
        application.addAppModule("Data_filtering",4000,10,100, 1);
        application.addAppModule("Data_processing",5500,10,100, 2);
        application.addAppModule("Event_handler",3000, 10, 200, 3);

        application.addAppEdge("Heart_Sensor", "Client", 1000, 500, "Heart_Sensor", Tuple.UP, AppEdge.SENSOR,999, 1);
        application.addAppEdge("Client", "Data_filtering", 4000, 500, "raw_heart", Tuple.UP, AppEdge.MODULE,5, 2);
        application.addAppEdge("Data_filtering", "Data_processing", 5500, 1500, "filtered_heart", Tuple.UP, AppEdge.MODULE,5, 3);
        application.addAppEdge("Data_processing", "Event_handler", 3000, 700, "analyzed_heart", Tuple.UP, AppEdge.MODULE,5, 4);
        //application.addAppEdge("Event_handler", "Client", 1000, 500, "response_heart", Tuple.DOWN, AppEdge.MODULE,20);
        //application.addAppEdge("Client", "Display", 500, 500, "display_data", Tuple.DOWN, AppEdge.ACTUATOR,20);
        ...
        final AppLoop loop1 = new AppLoop(new ArrayList<String>(){{add("Heart_Sensor");add("Client");add("Data_filtering");add("Data_processing");add("Event_handler");add("Display");}});
