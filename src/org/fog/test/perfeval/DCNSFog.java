...
public class DCNSFog {
	...
	private static void createFogDevices(int userId, String appId) {
		...
		for(int i=0;i<numOfAreas;i++){
			FogDevice neighbor = addNeighbor("neighbor", userId, appId, proxy.getId());
			FogDevice fog = addArea(i+"", userId, appId, proxy.getId());
			List<NeighborInArea> neighbors = new ArrayList<NeighborInArea>();
			neighbors.add(neighbor.getSelfInfo());
			fog.setNeighbors(neighbors);
		}
	}

	private static FogDevice addArea(String id, int userId, String appId, int parentId){
		FogDevice router = createFogDevice("d-"+id, 2500, 4000, 10000, 10000, 1, 0.0, 107.339, 83.4333);
	...
	private static FogDevice addNeighbor(String id, int userId, String appId, int parentId){
		FogDevice neighbor = createFogDevice("neighbor", 2500, 4000, 10000, 10000, 1, 0.0, 107.339, 83.4333);
		fogDevices.add(neighbor);
		neighbor.setUplinkLatency(2); // latency of connection between router and proxy server is 2 ms
		neighbor.setParentId(parentId);
		return neighbor;
	}
	...
	private static Application createApplication(String appId, int userId){
		 ...
		 */
		application.addAppModule("object_detector", 10, 0);
		application.addAppModule("motion_detector", 10, 1);
		application.addAppModule("object_tracker", 10, 2);
